...
export async function handleTelegramUpdate(update) {
  ...
  if (command === '/start') {
    await sendTelegramMessage(
      ...
      [
        'SolRadar is online.',
        '',
        'I monitor Birdeye trending tokens, score token risk, and send breakout alerts.',
        '',
        getHelpMessage(),
      ].join('\n')
    );
  ...
  if (command === '/trending') {
    await replyTrending(chatId);
    return { handled: true, command: 'trending' };
  }

  if (command === '/new') {
    await replyNewListings(chatId);
    return { handled: true, command: 'new' };
  }

  if (command === '/safe') {
    await replySafeCandidates(chatId);
    return { handled: true, command: 'safe' };
  }

  if (command === '/token') {
    await replyTokenLookup(chatId, args[0]);
