...
async function replyCompare(chatId, addr1, addr2) {
  ...
  return withBirdeyeFallback(chatId, async () => {
    ...
    const chg1 = Number(ov1?.priceChange24hPercent || 0);
    const chg2 = Number(ov2?.priceChange24hPercent || 0);
    ...
    const lines = [
      ...
      '',
      `Verdict: ${risk1.score >= risk2.score ? name1 : name2} scores higher on safety.`,
      '',
      '⚠️ Risk scores are simple heuristic checks, not financial advice. Always DYOR.',
    ];
...
async function replyTop(chatId) {
  return withErrorHandling(chatId, 'top', async () => {
    ...
    for (let i = 0; i < top3.length; i++) {
      ...
      const risk = calculateRiskScore(security, t);
