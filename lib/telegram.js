...
export async function handleTelegramUpdate(update) {
  ...
  if (command === '/token') {
    if (args[0] && !SOLANA_ADDRESS_PATTERN.test(args[0])) {
  ...
  if (command === '/status') {
    ...
    await sendTelegramMessage(
      ...
      [
        ...
        `Alert destination: ${status.hasChatId ? 'configured' : 'missing'}`,
        `Setup protection: ${status.hasSetupSecret ? 'enabled' : 'missing'}`,
        `Dashboard URL: ${status.hasPublicAppUrl ? 'configured' : 'missing'}`,
      ].join('\n')
    );
