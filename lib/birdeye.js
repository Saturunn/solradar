const BASE_URL = 'https://public-api.birdeye.so';
const REQUEST_TIMEOUT_MS = 12000;

const headers = {
  'X-API-KEY': process.env.BIRDEYE_API_KEY,
  'x-chain': 'solana',
};

class BirdeyeApiError extends Error {
  constructor(message, details = {}) {
    super(message);
    this.name = 'BirdeyeApiError';
    this.details = details;
  }
}

async function fetchBirdeyeJson(path) {
  const controller = new AbortController();
  const timeoutId = setTimeout(() => controller.abort(), REQUEST_TIMEOUT_MS);

  try {
    const res = await fetch(`${BASE_URL}${path}`, {
      headers,
      signal: controller.signal,
    });

    const contentType = res.headers.get('content-type') || '';

    if (!res.ok) {
      throw new BirdeyeApiError(`Birdeye request failed with status ${res.status}.`, {
        status: res.status,
        contentType,
      });
    }

    if (!contentType.toLowerCase().includes('application/json')) {
      throw new BirdeyeApiError('Birdeye returned a non-JSON response.', {
        status: res.status,
        contentType,
      });
    }

    try {
      return await res.json();
    } catch (error) {
      throw new BirdeyeApiError('Birdeye returned malformed JSON.', {
        cause: error.message,
      });
    }
  } catch (error) {
    if (error.name === 'AbortError') {
      throw new BirdeyeApiError('Birdeye request timed out.');
    }

    if (error instanceof BirdeyeApiError) {
      throw error;
    }

    throw new BirdeyeApiError(error.message || 'Birdeye request failed.');
  } finally {
    clearTimeout(timeoutId);
  }
}

async function getBirdeyeData(path, selector, fallbackValue) {
  const payload = await fetchBirdeyeJson(path);
  const result = selector(payload);
  return result ?? fallbackValue;
}

// GET trending tokens
export async function getTrendingTokens(limit = 20) {
  return getBirdeyeData(
    `/defi/token_trending?sort_by=rank&sort_type=asc&offset=0&limit=${limit}`,
    (payload) => payload?.data?.tokens,
    []
  );
}

// GET new listings — tries v2 endpoint, falls back to recently-created trending tokens
export async function getNewListings(limit = 20) {
  try {
    const result = await getBirdeyeData(
      `/defi/v2/tokens/new_listing?limit=${limit}&meme_platform_enabled=true`,
      (payload) => payload?.data?.items,
      []
    );
    if (result && result.length > 0) return result;
  } catch (_err) {
    // v2 endpoint may not be available on free tier
  }

  // Fallback: use trending tokens sorted by creation time as "new" listings
  return getBirdeyeData(
    `/defi/token_trending?sort_by=rank&sort_type=asc&offset=0&limit=${limit}`,
    (payload) => payload?.data?.tokens,
    []
  );
}

// GET token security info
export async function getTokenSecurity(address) {
  return getBirdeyeData(
    `/defi/token_security?address=${address}`,
    (payload) => payload?.data,
    null
  );
}

// GET token overview (price, volume, etc)
export async function getTokenOverview(address) {
  return getBirdeyeData(
    `/defi/token_overview?address=${address}`,
    (payload) => payload?.data,
    null
  );
}

// GET multiple token prices
export async function getMultiPrice(addresses) {
  const list = addresses.join(',');
  return getBirdeyeData(
    `/defi/multi_price?list_address=${list}`,
    (payload) => payload?.data,
    {}
  );
}

// GET 24h price history for sparkline
export async function getTokenPriceHistory(address, intervalType = '15m') {
  const now = Math.floor(Date.now() / 1000);
  const oneDayAgo = now - 86400;
  return getBirdeyeData(
    `/defi/history_price?address=${address}&address_type=token&type=${intervalType}&time_from=${oneDayAgo}&time_to=${now}`,
    (payload) => payload?.data?.items,
    []
  );
}

// GET token trade activity (recent trades)
export async function getTokenTradeData(address, limit = 10) {
  return getBirdeyeData(
    `/defi/txs/token?address=${address}&tx_type=swap&sort_type=desc&limit=${limit}`,
    (payload) => payload?.data?.items,
    []
  );
}

export function isBirdeyeApiError(error) {
  return error instanceof BirdeyeApiError;
}

// Risk scoring based on on-chain security heuristics + liquidity analysis
export function calculateRiskScore(security, tokenMeta = null) {
  if (!security) return { score: null, label: 'N/A', color: 'gray', flags: [] };

  let score = 100;
  const flags = [];

  // Authority checks
  if (security.freezeAuthority) { score -= 20; flags.push('Freeze Authority'); }
  if (security.mintAuthority) { score -= 20; flags.push('Mint Authority'); }

  // Holder concentration
  const top10 = Number(security.top10HolderPercent || 0);
  if (top10 > 80) { score -= 20; flags.push('Whale Concentration >80%'); }
  else if (top10 > 60) { score -= 10; flags.push('High Holder Concentration >60%'); }

  // Creator holding check
  const creatorPct = Number(security.creatorPercentage || 0);
  if (creatorPct > 50) { score -= 15; flags.push(`Creator holds ${creatorPct.toFixed(0)}%`); }
  else if (creatorPct > 20) { score -= 5; flags.push(`Creator holds ${creatorPct.toFixed(0)}%`); }

  // Liquidity-to-FDV ratio check (from trending token metadata)
  if (tokenMeta) {
    const liq = Number(tokenMeta.liquidity || 0);
    const fdv = Number(tokenMeta.fdv || tokenMeta.marketcap || 0);
    if (fdv > 0 && liq > 0) {
      const ratio = liq / fdv;
      if (ratio < 0.02) { score -= 15; flags.push('Very Low Liquidity Ratio'); }
      else if (ratio < 0.05) { score -= 5; flags.push('Low Liquidity Ratio'); }
    }
  }

  // Token standard bonus
  if (!security.isToken2022) { score += 5; }

  score = Math.max(0, Math.min(100, score));

  let label;
  let color;
  if (score >= 80) { label = 'SAFE'; color = 'green'; }
  else if (score >= 60) { label = 'CAUTION'; color = 'yellow'; }
  else if (score >= 40) { label = 'RISKY'; color = 'orange'; }
  else { label = 'DANGER'; color = 'red'; }

  return { score, label, color, flags };
}
